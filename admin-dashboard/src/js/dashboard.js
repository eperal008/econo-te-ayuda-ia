document.addEventListener('DOMContentLoaded', () => {
        ...
        try { 
            ...
            products.forEach(product => { 
                ...
                row.innerHTML = ` 
                    ...
                    <td data-label="Name (ES)">${product.name_es || ''}</td> 
                    <td data-label="Category">${product.category || ''}</td> 
                    <!-- <td data-label="Brand">${product.brand || ''}</td> --> 
                    <td data-label="Aisle">${product.aisle || ''}</td> 
            ...
            productTableBody.innerHTML = `<tr><td colspan="5" class="text-center p-4 text-red-500">Error loading products: ${error.message}</td></tr>`; 
        } 
    };
    ...
    if(addProductForm) addProductForm.addEventListener('submit', async (e) => { 
        ...
        try { 
            ...
            if(error.message.includes('Failed to add product')) alert(`Error adding product: ${error.message}`); 
        } 
    });
    ...
    if(saveEditButton) saveEditButton.addEventListener('click', async () => { 
        try { 
            ...
            if(error.message.includes('Failed to update product')) alert(`Error updating product: ${error.message}`); 
        } 
    });
    ...
    if(productTableBody) productTableBody.addEventListener('click', (e) => { 
        ...
        if (targetButton.classList.contains('delete-button')) { 
            if (confirm(`Are you sure you want to delete "${product.name_es}" (ID: ${product.id})?`)) { 
    ...
    if(restartBotButton) restartBotButton.addEventListener('click', async () => { 
        if (confirm("Are you sure you want to restart the bot? This will cause a brief interruption of service (about 30-60 seconds).")) { 
            try { 
                const response = await fetch(`${API_BASE_URL}/api/admin/restart-bot`, { 
                    method: 'POST', 
                    headers: { 'x-api-key': apiKey } 
                }); 
                const result = await response.json(); 
